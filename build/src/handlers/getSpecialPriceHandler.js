...
const getBasePriceController_1 = __importDefault(
  require("../controllers/getBasePriceController")
);
  ...
  try {
    ...
    if (!user || !user.metadata || !user.metadata.precios_especiales) {
      res
        .status(404)
        .json({
          error: "cliente no encontrado o no tiene precios especiales.",
        });
      return;
    }
    const specialPrice =
      (_a = user === null || user === void 0 ? void 0 : user.metadata) ===
        null || _a === void 0
        ? void 0
        : _a.precios_especiales.find(
            (price) => price.nombre_producto === productName
          );
    if (!specialPrice) {
      const basePrice = await (0, getBasePriceController_1.default)(
        productName
      );
